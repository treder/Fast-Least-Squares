...
% dataset. Uses multi-class LDA.
%
% Requires: Fieldtrip and the MVPA-Light toolbox.

function WakemanHenson_run_permutations_multiclass_LDA(ss)
...
win = 0.2; % averaging window size in seconds
...
% for ss=1%:numel(sbjs)
    ...
    % 
    % To obtain a multiclass problem, we split the faces into two separate
    % classes: 5,6,7 vs 13,14,15
    clabel = dat.trialinfo;
    clabel(ismember(clabel,[ 5, 6, 7])) = 1;  % faces 1
    clabel(ismember(clabel,[ 13, 14, 15])) = 2;  % faces 2
    clabel(ismember(clabel,[ 17,18,19])) = 3;  % scrambled
...
%     acc = mv_crossvalidate([], X, clabel);
    ...
    %% Save data
    save([resultsdir 'WakemanHenson_multiclass_LDA_sbj' num2str(ss)],'time1','time2','cfg', ...
